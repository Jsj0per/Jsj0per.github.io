...
최근 알고리즘 문제를 풀 시간이 없어서 잔디심기용으로 쉬운문제들을 리마인드 하는 마음으로 풀기로 하였다.  
물론 진짜 그냥 풀면 실력에 아무런 이득도 없으니 브론즈 문제들은 다른 언어로 풀어보는 소일거리를 해보기로 하였다.  
...
[답안 링크](http://boj.kr/2446f3e1caf34ab89a286c2eefd6ef21)
...
(1..28).each do |i| # 28번 반복한다
  num = gets.chomp.to_i # 숫자를 입력받는다.
  num_list.push(num) # 입력 받은 숫자를 리스트에 담는다
end

(1..30).each do |j| # 30번 반복한다.
  if not num_list.include?(j) # 만약 리스트 안에 없다면
    puts "#{j} " # 출력한다.
  end
end
```
...
~~저요~~.
ruby에서는 하나씩 받아서 넣어야할 때는 to_변수 이렇게 붙여야 한다는 점을 풀면서 알게 되었다.  
은근 파이썬과 비슷한듯 하면서, 꽤 다른 언어라는 점을 다시 한 번 느꼈다.  
